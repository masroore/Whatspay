...
if(!function_exists('multiFile')) {
    ...
    function multiFile($files = array(),$dimensions)
    {
...
if (!function_exists('createSlug')) {
    function createSlug($data){
        ...
        $title=($data->name)?$data->name:$data->store_name;
...
if (!function_exists('getIdBySlug')) {
    function getIdBySlug($slug,$model){
        $id=\App\Models\Slug::where('slugable_type',$model)
...
if (!function_exists('getDeliveryTimings')){
    function getDeliveryTimings($hr = 0, $min = 0, $days = 0){
        ...
        if($hr > 0 || $min > 0){
            ...
            return array('timing' => $delivery_time, 'type' => 'min');

        }else if($days > 0){

            $type = ($days > 1) ? ' Days' : ' Day';
            return array('timing' => $days, 'type' => $type);
