...
class PublicService {
    ...
    public function homeListing() {
        try{
            ...
            $stores = Store::select(
                ...
                'images', 'description', 'delivery_hours', 'delivery_minutes'
            )->where([
                'is_approved' => 1,
                'status' => 1
            ])->with([
    ...
    public function search($keyword) {
        try {
            ...
            $stores = Store::select(
                'stores.id', 'store_name', 'business_url', 'whatsapp_num', 'store_logo',
                'images', 'description', 'delivery_hours', 'delivery_minutes', 'store_timings', 'store_timings as delivery_time'
            )
                ->where([
                    'is_approved' => 1,
                    'stores.status' => 1
    ...
    public function getStore($url) {
        ...
        try {
            $store = Store::select(
                'id', 'store_logo', 'store_name',
                'whatsapp_num', 'business_url', 'description', 'images', 'email', 'website',
                'ntn_num', 'street_address', 'latitude', 'longitude', 'country',
                'is_online', 'postal_code', 'area', 'bank_phone', 'payment_method',
                'service_options', 'applicable_range', 'currency', 'delivery_hours', 'cash_on_delivery as payment_method',
                'delivery_minutes', 'store_timings', 'shipping_percentage as delivery_fee',
                'store_timings as delivery_time', 'shipping_percentage_type'
            )
                ->where([
                    'business_url' => $url,
                    'is_approved' => 1,
    ...
    public function categoryProducts($store, $category) {
        try {
            $products = Products::select(
                'id', 'store_id', 'name', 'description', 'status',
                'images', 'sku', 'price', 'with_storehouse_management', 'quantity',
                'category_id', 'brand_id', 'is_variation', 'sale_price'
            )
                ->where([
                    'store_id' => $store,
