...
class UserService {
     ...
     *
     */
    protected $userRepository, $storeRepository ,$employeeRepository, $favoritesRepository;
    ...
    public function register(Request $request) {
        ...
        try{
			...
			if($request['user_type'] == 'company' && $user) {
            ...
            $validator = Validator::make($request->input(), [
                'username'  => ['required',
                    function ($attribute, $value, $fail) {
                        if(!filter_var($value, FILTER_VALIDATE_EMAIL) && strtoupper(substr($value, 0, 3)) != 'WP-'){
    ...
    public function resendActivation(Request $request) {
        try{
            ...
            $validator = Validator::make($request->input(), [
                'username' => ['required', 'string', function ($attribute, $value, $fail) {
                    if(!filter_var($value, FILTER_VALIDATE_EMAIL)) {
                        ...
                        if($value <= 0) $fail('The '.$attribute.' is invalid.');
                    }
                }]
            ]);
            ...
            if($user->user_status != 0 && $user->is_user_deactivated == 0) {
    ...
    public function checkUserStatus($user) {
        ...
        if (($user->user_type == 'company' || $user->user_type == 'employee') && $stores->isNotEmpty()) {
